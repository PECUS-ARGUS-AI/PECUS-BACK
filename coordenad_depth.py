...
def run_depth(
    ...
    # simula os argumentos vindos do terminal
    sys.argv = [
        "coordenad_depth.py",
        "--img-path", img_path,
        "--outdir", outdir,
    ...
    if pred_only:
        sys.argv.append("--pred-only")
    if grayscale:
        sys.argv.append("--grayscale")
    if save_pointcloud:
        sys.argv.append("--save-pointcloud")
    if focal_length is not None:
        sys.argv.extend(["--focal-length", str(focal_length)])

    # roda o código original
    ...
    # A PARTIR DAQUI É SEU CÓDIGO ORIGINAL
    # Cole aqui TODO o código que estava no `if __name__ == '__main__'`
